...
1. **Given** the ApplicationSet is deployed, **When** a new `apps/my-app.yaml` is added to the infra repo, **Then** ArgoCD automatically creates an Application for it within the sync interval
...
- **FR-003**: System MUST apply the ApplicationSet manifest from `infra/argocd-infra/applicationset.yaml` to the `argocd` namespace
...
- **FR-005**: System MUST preserve the existing bootstrap behavior (k3d cluster creation, ArgoCD installation, port configuration, password change)
...
- **SC-005**: All three infrastructure apps (vault, prometheus, external-secrets) are accessible via their respective services after bootstrap completes
